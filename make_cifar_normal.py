...
def make_cifar_normal(PATH, NEW_PATH):
    ...
    for i, batch in enumerate(dataloaders['val']):
        ...
        inputs = inputs*255 # need this to save as png without problem
        
        image = np.moveaxis(np.squeeze(inputs), 0, 2)
        im = Image.fromarray(image.astype('uint8')).convert('RGB')
        loc = str(SAVE_PATH) + '/val_'+str(i)+'.png'
        im.save(loc)
        
        files['val'] = files['val'].append({'path': loc, 'class': np.squeeze(target)}, ignore_index=True)
    ...
    for i, batch in enumerate(dataloaders['train']):
        ...
        files['train'] = files['train'].append({'path': loc, 'class': np.squeeze(target)}, ignore_index=True)
...
if __name__ == '__main__':
    make_cifar_normal(args.PATH, args.NEW_PATH)